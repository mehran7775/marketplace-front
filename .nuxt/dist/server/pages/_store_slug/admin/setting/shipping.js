...
//
//
/* harmony default export */ var shippingvue_type_script_lang_js_ = ({name:"shipping",components:{PageTitle: pageTitle["default"]},layout:"main-content",data(){return{one:1,zero:0,store:{shipping_setting:{}}};},created(){this.getData();},methods:{getData(){api["a" /* default */].get('store/find/'+this.$route.params.store_slug,this.$cookies.get('token')).then(res=>{this.store=res.data.data;});},updateSetting(){api["a" /* default */].post('store/update-shipping/'+this.$route.params.store_slug,this.store.shipping_setting,this.$cookies.get('token')).then(response=>{alert(response.data.message);this.getData();}).catch(({response})=>{alert(response.data.data[Object.keys(response.data.data)[0]]);});}}});
// CONCATENATED MODULE: ./pages/_store_slug/admin/setting/shipping.vue?vue&type=script&lang=js&
 /* harmony default export */ var setting_shippingvue_type_script_lang_js_ = (shippingvue_type_script_lang_js_); 
// EXTERNAL MODULE: ./node_modules/vue-loader/lib/runtime/componentNormalizer.js
var componentNormalizer = __webpack_require__(1);

// CONCATENATED MODULE: ./pages/_store_slug/admin/setting/shipping.vue
...
var component = Object(componentNormalizer["a" /* default */])(
  setting_shippingvue_type_script_lang_js_,
  render,
  staticRenderFns,
  false,
  injectStyles,
